...
_Skip this section if you want to install from a different source such as the bundle provided by the Apache Software Foundation._
...
#### Install Apache Tomcat from tar file

_You may skip this section if you installed Tomcat from an RPM. It is possible to install multiple versions of Tomcat on your server and have different instances use the different Tomcat versions, so this section may still be beneficial._

If you do not want to use a EuPathDB RPM (perhaps we have not packaged the Tomcat version you want), you can download the binary tar file from the Apache Software Foundation and extract into /usr/local/ . 

If you install your own Tomcat you will need to compile `jsvc` binary (this is included with our RPMs).
